...
- 工作区**就是 git 仓库**。`forge new` 自动 `git init` + 第一次 commit。
- approved 基线 = `git rev-parse HEAD`。不再有独立的 `.forge/approved/sp/` 拷贝。
- `forge diff` = `git diff HEAD -- sp/` + 重渲 `output/` 跟 HEAD 版本对比预览。
- `forge approve` = 重建 `output/` → `git add sp/ output/` → `git commit -m "<note>" --trailer forge-provenance:<...>`。把绑定的外部 target（如 `~/.claude/CLAUDE.md`）自动同步。
- `forge reject` = `git restore --source=HEAD --staged --worktree -- sp/ output/`。
- `forge rollback <hash>` = `git checkout <hash> -- sp/ output/`。任意历史 hash，不是只能回最近一次。
- `forge changelog` = 现场从 `git log -- sp/` 渲染。也能直接 `git log` / `lazygit` / GitHub UI 看。
- Provenance：每个 output 文件顶部有 stable digest + per-section 来源（deterministic）；commit message trailer 里有 `forge-provenance: version=0.2.0 source=...` 标记 forge 自己产生的 commit（区别于用户手动改 sp/ 后自己 git commit 的）。
