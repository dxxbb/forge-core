...
v0.1 把五大 pillar 都 ship 到"可看可跑"的程度，不是只做了 Context Compiler 一个：

| Pillar | 做到什么 |
|---|---|
| **Canonical Source** | markdown section + frontmatter + MVC 分层（Config 砍掉内容字段） |
| **Governance** | 审核关口完整（diff/approve/reject/doctor），watcher/inbox/rollback 有 stub |
| **Context Compiler** | 2 个 core adapter、provenance、schema、确定性编译 |
| **Evaluation** | 结构 bench 完整，行为 eval 框架 + Anthropic runner/judge |
| **Adapters** | 2 core + 3 contrib（Cursor / Codex CLI / rulesync bridge） |

88 个单测，对真实 dxyOS 工作区跑过端到端（91.5% 逐行保留率 + 行为 A/B 2-2 打平）。

**还没做的**：
- 没有真 daemon watcher。`forge watch` 是手动一次性扫描。（v0.2）
- 没有 LLM 打分的大规模 eval。现在 bench 是结构的，真 LLM eval 只跑过 4 个任务。（v0.3）
- 没有 Mem0 / Letta / Zep 真集成。源头就是 markdown 文件。（v0.4，症状驱动）
- 没有 CI 集成、没有托管版、没有 Web UI。

这些都是故意不做的。整件事的论点是：**真正难的是关口 + 分层 + bench 契约，不是编译本身**。v0.1 先把论点以最小版本 ship 出来，让我（以及任何觉得这个方向有意思的人）先验证概念，再往上加。每个 pillar 的 spec 文档里都写了"v0.1 做到什么、明确不做什么、v0.2/0.3/0.4 要补什么"——不是 roadmap 空头支票。
